...
type HolstAnswer struct {
	HolstNumbersSumm map[string]float64
	HolstOperators map[string]int
	HolstOperands map[string]int
...
func run(pass *analysis.Pass) (interface{}, error) {
	...
	inspect.Preorder(nodeFilter, func(n ast.Node) {
		...
		case *ast.FuncDecl:
			...
			jsonData,_ := json.Marshal(halstMet)
			os.WriteFile("C:\\Users\\Lenovo\\GolandProjects\\awesomeProject\\Data\\data.json",jsonData,0644)
			...
			maintIdx := calcMaintIndex(halstMet.HolstNumbersSumm["volume"], cycloComp, loc)
			...
			if flag.Lookup("test.v") != nil {
				pass.Reportf(n.Pos(), "Cyclomatic complexity: %d, Halstead difficulty: %0.3f, volume: %0.3f", cycloComp, halstMet.HolstNumbersSumm["difficulty"], halstMet.HolstNumbersSumm["volume"])
			}
		}
	})
...
func calcHalstComp(fd *ast.FuncDecl) HolstAnswer {
	operators, operands := map[string]int{}, map[string]int{}
	halstMet := map[string]float64{}
	HalstOper := map[string]int{}
	HalstOperands := map[string]int{}
	...
	for operator, val := range operators {
		HalstOper[operator] = val
		sumOpt += val
	}

	for operand, val := range operands {
		sumOpd += val
		HalstOperands[operand] = val
	}
	...
	halstMet["NAtAllOperators"] = float64(sumOpt)
	halstMet["NAtAllOperands"] = float64(sumOpd)
	halstMet["NUniqOperators"] = float64(distOpt)
	halstMet["NUniqOperands"] = float64(distOpd)
	halstMet["length"] = float64(length)
	halstMet["ProgramDict"] = float64(nVocab)
	answer := HolstAnswer{
		HolstNumbersSumm: halstMet,
		HolstOperators:   HalstOper,
		HolstOperands:    HalstOperands,
	}
	return answer
}
